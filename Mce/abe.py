...
# RIGHTPEER START
ABE_APPNAME = "RightPeer Explorer"
ABE_VERSION = __version__
ABE_URL = 'https://github.com/rightpeer/rightpeer-explorer'

COPYRIGHT_YEARS = '2011-2017'
COPYRIGHT = "Coin Sciences Ltd and Abe developers"
COPYRIGHT_URL = 'https://github.com/rightpeer/rightpeer-explorer'
# RIGHTPEER END
...
<body>
    <div class="container">
	<table><td>
	<a title="Back to home" href="%(dotdot)s%(HOMEPAGE)s"><img src="%(dotdot)s%(STATIC_PATH)slogo32.png" alt="RightPeer logo" /></a>
...
# RIGHTPEER START
    ...
    def get_blockchainparams(abe, chain):
        ...
        rightpeer_name = abe.store.get_rightpeer_name_by_id(chain.id)
        params = {}
        try:
            params = util.jsonrpc(rightpeer_name, url, "getblockchainparams")
...
# RIGHTPEER START
                ...
                body += '<td>'
                if chain.__class__.__name__ is "RightPeer":
                    if num_assets == -1:
                        body += '?'
                    elif num_assets>=0:
...
# RIGHTPEER_START
    def handle_chain(abe, page):
        ...
        params_resp = None
        try:
            info_resp = util.jsonrpc(rightpeer_name, url, "getinfo")
            params_resp = util.jsonrpc(rightpeer_name, url, "getblockchainparams")
...
# RIGHTPEER START
            # Describe RightPeer specific transaction
            labels = []
            labeltype = 'success'
            try:
            ...
            if mytx is not None:
                for txout in mytx['out']:
                    ...
                    if script_type in [Chain.SCRIPT_TYPE_RIGHTPEER, Chain.SCRIPT_TYPE_RIGHTPEER_P2SH]:
                        data = util.get_rightpeer_op_drop_data(txout['binscript'])
                        if data is not None:
                            opdrop_type, val = util.parse_op_drop_data(data, chain)
                            label = util.get_op_drop_type_description(opdrop_type)
                        else:
                            label = 'Unknown RightPeer command'
                            labeltype = 'danger'

                    elif script_type is Chain.SCRIPT_TYPE_RIGHTPEER_OP_RETURN:
                        opreturn_type, val = util.parse_op_return_data(data, chain)
                        label = util.get_op_return_type_description(opreturn_type)

                    elif script_type is Chain.SCRIPT_TYPE_RIGHTPEER_STREAM:
                        label = 'Create Stream'

                    elif script_type is Chain.SCRIPT_TYPE_RIGHTPEER_FILTER:
                        label = 'Create Filter'

                    elif script_type is Chain.SCRIPT_TYPE_RIGHTPEER_STREAM_ITEM:
                        label = 'Stream Item'

                    elif script_type is Chain.SCRIPT_TYPE_RIGHTPEER_ENTITY_PERMISSION:
                        label = 'Approval or Entity Permission'

                    elif script_type in [Chain.SCRIPT_TYPE_RIGHTPEER_SPKN, Chain.SCRIPT_TYPE_RIGHTPEER_SPKU]:
                        data = util.get_rightpeer_op_drop_data(txout['binscript'])
                        if data is not None:
                            opdrop_type, val = util.parse_op_drop_data(data, chain)
                            label = util.get_op_drop_type_description(opdrop_type)
                    ...
                    elif script_type is Chain.SCRIPT_TYPE_RIGHTPEER_SPKF:
                        label = "Inline Data"

                    elif script_type is Chain.SCRIPT_TYPE_RIGHTPEER_APPROVE:
...
# RIGHTPEER START
    ...
    def show_tx_row_to_html_impl(abe, chain, body, asset_txid_dict, binscript, script_type, data, v_json, data_ref):
        ...
        msgtype = 'success'
        msgpanelstyle = ''
        if script_type in [Chain.SCRIPT_TYPE_RIGHTPEER,
                           Chain.SCRIPT_TYPE_RIGHTPEER_P2SH,
                           Chain.SCRIPT_TYPE_RIGHTPEER_FILTER,
                           Chain.SCRIPT_TYPE_RIGHTPEER_STREAM,
                           Chain.SCRIPT_TYPE_RIGHTPEER_STREAM_ITEM,
                           Chain.SCRIPT_TYPE_RIGHTPEER_SPKN,  # also matches template used for input cache opdrop
                           Chain.SCRIPT_TYPE_RIGHTPEER_SPKF,
                           Chain.SCRIPT_TYPE_RIGHTPEER_SPKU,
                           Chain.SCRIPT_TYPE_RIGHTPEER_APPROVE]:
            # NOTE: data returned above is pubkeyhash, due to common use to get address, so we extract data ourselves.
            data = util.get_rightpeer_op_drop_data(binscript)
            if data is not None:
                opdrop_type, val = util.parse_op_drop_data(data, chain)
                if opdrop_type==util.OP_DROP_TYPE_ISSUE_ASSET:
                    ...
                    msg = 'Create stream:'
                    if chain.protocol_version < 10007:
                        data = util.get_rightpeer_op_return_data(binscript)
                        opreturn_type, val = util.parse_op_return_data(data, chain)
                        fields = val['fields']
                    else:
                ...
                elif opdrop_type==util.OP_DROP_TYPE_SPKE and script_type==Chain.SCRIPT_TYPE_RIGHTPEER_SPKU:
                ...
                elif (opdrop_type==util.OP_DROP_TYPE_SPKE and script_type==Chain.SCRIPT_TYPE_RIGHTPEER_STREAM_ITEM) or opdrop_type == util.OP_DROP_TYPE_STREAM_ITEM:
                ...
                elif opdrop_type == util.OP_DROP_TYPE_SPKE and script_type == Chain.SCRIPT_TYPE_RIGHTPEER_APPROVE:
                    msg = "Approve upgrade"
                    msg += '<table class="table table-bordered table-condensed">'
                    try:
                        resp = abe.store.list_upgrades(chain)
        ...
        if script_type is Chain.SCRIPT_TYPE_RIGHTPEER_ENTITY_PERMISSION:
        ...
        if script_type is Chain.SCRIPT_TYPE_RIGHTPEER_OP_RETURN:
            opreturn_type, val = util.parse_op_return_data(data, chain)
    ...
    def show_tx(abe, page, tx):
        ...
        def row_to_html(v, row, this_ch, other_ch, no_link_text):
            ...
            if binscript is not None:
                ...
                if script_type in [Chain.SCRIPT_TYPE_RIGHTPEER_OP_RETURN,
                                   Chain.SCRIPT_TYPE_RIGHTPEER_FILTER,
                                   Chain.SCRIPT_TYPE_RIGHTPEER_STREAM,
                                   Chain.SCRIPT_TYPE_RIGHTPEER_STREAM_ITEM,
                                   Chain.SCRIPT_TYPE_RIGHTPEER_SPKN,
                                   Chain.SCRIPT_TYPE_RIGHTPEER_SPKF,
                                   Chain.SCRIPT_TYPE_RIGHTPEER_SPKU]:
                    novalidaddress = True
...
# RIGHTPEER END
        ...
        chain_name = abe.store.get_rightpeer_name_by_id(chain.id)
...
# RIGHTPEER START
        ...
        body += [' <a role="button" class="btn btn-default btn-xs" href="../rpctxjson/', tx['hash'], '">RightPeer JSON</a>']
        body += [' <a role="button" class="btn btn-default btn-xs" href="../rpctxraw/', tx['hash'], '">RightPeer Hex</a>']
        body += ['</p>']
# RIGHTPEER END
    ...
    def show_mempool_tx_json(abe, page, tx):
        ...
        def row_to_html(v, row, this_ch, other_ch, no_link_text, tx):
            ...
            if addressLabel is not 'None':
                version, pubkeyhash = util.decode_address_rightpeer(addressLabel)
...
# RIGHTPEER START
    ...
    def handle_address(abe, page):
        ...
        # check the address
        version, pubkeyhash = util.decode_check_address_rightpeer(address)
        if pubkeyhash is None:
            raise PageNotFound()
            #raise MalformedAddress("Invalid address")

        # If the HTML link for this handler gets only created for RightPeer networks, we don't need to check class.
        #if chain.__class__.__name__ is "RightPeer":
        body += ['<h3>Permissions</h3>']
        try:
            resp = util.jsonrpc(rightpeer_name, url, "listpermissions", "all", address)
            if len(resp) > 0 :
                body += ['<ul>']
        ...
        if abe.get_blockchainparams(chain).get('initial-block-reward', 0) > 0:
            ...
            try:
                resp = util.jsonrpc(rightpeer_name, url, "getaddressbalances", address)
                if len(resp) is 0:
                    body += ['None']
                else:
                    body += ['<ul>']
    ...
    def handle_assetaddress(abe, page):
        ...
        try:
            resp = util.jsonrpc(rightpeer_name, url, "listassets", assetref)
            asset = resp[0]
        except util.JsonrpcException as e:
            msg= "JSON-RPC error({0}): {1}".format(e.code, e.message)
        ...
        def get_asset_amount_from_txinout(tx, this_ch, other_ch, asset, chain):
            ...
            if checksum is None:
                ...
                addr = util.hash_to_address_rightpeer(vers, binaddr, checksum)
            ...
            if this_ch=='i':
                binscript = tx['rightpeer_scriptPubKey']

            script_type, data = chain.parse_txout_script(binscript)

            if script_type not in [Chain.SCRIPT_TYPE_RIGHTPEER, Chain.SCRIPT_TYPE_RIGHTPEER_P2SH]:
                return 0
    ...
    def handle_assetref(abe, page):
        ...
        # get asset information and issue tx as json
        try:
            resp = util.jsonrpc(rightpeer_name, url, "listassets", assetref, True) # verbose to get 'issues' field
            asset = resp[0]
            issuetxid = asset['issuetxid']
            resp = util.jsonrpc(rightpeer_name, url, "getrawtransaction", issuetxid, 1)
            issuetx = resp
        except util.JsonrpcException as e:
            msg= "JSON-RPC error({0}): {1}".format(e.code, e.message)
            body += ['<div class="alert alert-danger" role="warning">', msg ,'</div>']
...
#                                                                                                                '' b['height'] if b['height'] is not None else '')
        ...
        body += ['<p class="text-right">']
        body += ['<button href="#RawJson" class="btn btn-default btn-xs" data-toggle="collapse">RightPeer JSON</button>']
...
# RIGHTPEER START
            ...
            if checksum is None:
                ...
                address = util.hash_to_address_rightpeer(vers, pubkeyhash, checksum)
...
# RIGHTPEER END
    ...
    def handle_assets(abe, page):
        ...
        rightpeer_name = abe.store.get_rightpeer_name_by_id(chain.id)
        try:
            resp = util.jsonrpc(rightpeer_name, url, "listassets")
            num_assets = len(resp)
        except util.JsonrpcException as e:
            msg= "JSON-RPC error({0}): {1}".format(e.code, e.message)
            body += ['<div class="alert alert-danger" role="warning">', msg ,'</div>']
...
# RIGHTPEER START
                    ...
                    if checksum is None:
                        ...
                        other = util.hash_to_address_rightpeer(vers, binaddr, checksum)
# RIGHTPEER END
                    if other != address:
...
# RIGHTPEER START
            ...
            chain = abe.store.get_chain_by_id(1)
            version, binaddr = util.decode_check_address_rightpeer(address)
# RIGHTPEER END
        except Exception:
...
# RIGHTPEER START
    def search_address_prefix(abe, ap):
        ...
        def process(row):
            ...
            if hash is None:
                return None
            address = util.hash_to_address_rightpeer(address_version, hash, checksum)
...
# RIGHTPEER START
        version, hash = util.decode_check_address_rightpeer(addr)
# RIGHTPEER END
        if hash is None:
...
# RIGHTPEER START
    if checksum is None:
        ...
        addr = util.hash_to_address_rightpeer(version, hash, checksum)
# RIGHTPEER END
    if text is not None:
        visible = text
