...
# RIGHTPEER START
# Template to match a RightPeer permission or asset transaction which puts a payload before OP_DROP
SCRIPT_RIGHTPEER_TEMPLATE = [
...
# Template to match a RightPeer entity permission command
SCRIPT_RIGHTPEER_ENTITY_PERMISSION_TEMPLATE = [
...
# Template to match a RightPeer stream item
# spke for stream item
SCRIPT_RIGHTPEER_STREAM_ITEM_TEMPLATE = [ opcodes.OP_PUSHDATA4, opcodes.OP_DROP, opcodes.OP_PUSHDATA4, opcodes.OP_DROP, opcodes.OP_RETURN, opcodes.OP_PUSHDATA4 ]
# spkf for stream item
SCRIPT_RIGHTPEER_STREAM_FORMATTED_ITEM_TEMPLATE = [ opcodes.OP_PUSHDATA4, opcodes.OP_DROP, opcodes.OP_PUSHDATA4, opcodes.OP_DROP, opcodes.OP_PUSHDATA4, opcodes.OP_DROP, opcodes.OP_RETURN, opcodes.OP_PUSHDATA4 ]

# spke for asset
SCRIPT_RIGHTPEER_FOLLOW_ON_ISSUANCE_METADATA_TEMPLATE = [ opcodes.OP_PUSHDATA4, opcodes.OP_DROP, opcodes.OP_PUSHDATA4, opcodes.OP_DROP, opcodes.OP_RETURN]

# Template to match a RightPeer create stream command
# or spkf for asset item
SCRIPT_RIGHTPEER_STREAM_TEMPLATE = [ opcodes.OP_PUSHDATA4, opcodes.OP_DROP, opcodes.OP_RETURN, opcodes.OP_PUSHDATA4 ]

# TODO: Rename as the template also matches spki for input cache
# spkn, for both new issue and create stream
SCRIPT_RIGHTPEER_SPKN_TEMPLATE = [opcodes.OP_PUSHDATA4, opcodes.OP_DROP, opcodes.OP_RETURN ]

# Matches all opcodes < PUSHDATA4
# See: https://en.bitcoin.it/wiki/Script
SCRIPT_RIGHTPEER_OP_RETURN_TEMPLATE = [ opcodes.OP_RETURN, opcodes.OP_PUSHDATA4 ]

# Template to match a RightPeer asset pay-to-script-hash (P2SH) output script
# e.g. "OP_HASH160 f1191c44953b7da866dba982d155bbd8eeb89dfc OP_EQUAL 73706b7104000000f70200001ae41027000000000000 OP_DROP"
SCRIPT_RIGHTPEER_P2SH_TEMPLATE = [ opcodes.OP_HASH160, PUBKEY_HASH_LENGTH, opcodes.OP_EQUAL, opcodes.OP_PUSHDATA4, opcodes.OP_DROP]
...
SCRIPT_TYPE_P2SH = 6
# RIGHTPEER START
SCRIPT_TYPE_RIGHTPEER = 7
SCRIPT_TYPE_RIGHTPEER_OP_RETURN = 8
SCRIPT_TYPE_RIGHTPEER_P2SH = 9
SCRIPT_TYPE_RIGHTPEER_STREAM = 10
SCRIPT_TYPE_RIGHTPEER_STREAM_ITEM = 11
SCRIPT_TYPE_RIGHTPEER_ENTITY_PERMISSION = 12
SCRIPT_TYPE_RIGHTPEER_SPKN = 13
SCRIPT_TYPE_RIGHTPEER_SPKU = 14    # follow on asset issuance metadata
SCRIPT_TYPE_RIGHTPEER_SPKF = 15
SCRIPT_TYPE_RIGHTPEER_APPROVE = 16
SCRIPT_TYPE_RIGHTPEER_FILTER = 17
# RIGHTPEER END
...
# RIGHTPEER START
        * SCRIPT_TYPE_RIGHTPEER - DATA is the binary public key (there is another method to get the OPDROP data)
        * SCRIPT_TYPE_RIGHTPEER_P2SH - DATA is the binary script hash (there is another method to get the OPDROP data)
        * SCRIPT_TYPE_RIGHTPEER_STREAM - DATA is a dicationary containing op_drop and op_return data.
        * SCRIPT_TYPE_RIGHTPEER_STREAM_ITEM - Data is a dictionary containing stream creation txid, item key, item data.
        * SCRIPT_TYPE_RIGHTPEER_STREAM_PERMISSION - Data is a dictionary containing stream creation txid, permissions, pubkey_hash
        * SCRIPT_TYPE_RIGHTPEER_SPKN
        * SCRIPT_TYPE RIGHTPEER_SPKE
        * SCRIPT_TYPE_RIGHTPEER_SPKF - DATA is the formatted metadata
# RIGHTPEER END
        """
        if script is None:
            raise ValueError()
...
# RIGHTPEER START
        # Return dict
        if deserialize.match_decoded(decoded, SCRIPT_RIGHTPEER_ENTITY_PERMISSION_TEMPLATE):
            if decoded[7][1].startswith("spkp"):
                dict = {"txid":decoded[5][1], "permissions":decoded[7][1], "pubkey_hash":decoded[2][1]}
                return SCRIPT_TYPE_RIGHTPEER_ENTITY_PERMISSION, dict


        # Return script type and address
        if deserialize.match_decoded(decoded, SCRIPT_RIGHTPEER_TEMPLATE):
            pubkey_hash = decoded[2][1]
            if len(pubkey_hash) == PUBKEY_HASH_LENGTH:
                return SCRIPT_TYPE_RIGHTPEER, pubkey_hash
        ...
        if len(decoded) >= 6 and decoded[2][1] and decoded[2][1].startswith("spkk"):
            ...
            return SCRIPT_TYPE_RIGHTPEER_STREAM_ITEM, d

        # Return dict
        if deserialize.match_decoded(decoded, SCRIPT_RIGHTPEER_STREAM_TEMPLATE):
            drop_data = decoded[0][1]
            if drop_data[:4] == "spkf":
                script_type = SCRIPT_TYPE_RIGHTPEER_SPKF
            else:
                script_type = SCRIPT_TYPE_RIGHTPEER_STREAM
            dict = {"op_drop": drop_data, "op_return": decoded[3][1]}
            return script_type, dict

        if deserialize.match_decoded(decoded, SCRIPT_RIGHTPEER_FOLLOW_ON_ISSUANCE_METADATA_TEMPLATE):
            # Could also be upgrade approval.
            if decoded[2][1].startswith("spka"):
                dict = {"upgradeid": decoded[0][1], "approved": decoded[2][1]}
                return SCRIPT_TYPE_RIGHTPEER_APPROVE, dict
            else:
                dict = {"assetidentifier":decoded[0][1], "assetdetails":decoded[2][1]}
                return SCRIPT_TYPE_RIGHTPEER_SPKU, dict

        #SCRIPT_RIGHTPEER_SPKN_TEMPLATE
        if deserialize.match_decoded(decoded, SCRIPT_RIGHTPEER_SPKN_TEMPLATE):
            metadata = decoded[0][1]
            return SCRIPT_TYPE_RIGHTPEER_SPKN, metadata

        # Return script type and metadata byte array
        elif deserialize.match_decoded(decoded, SCRIPT_RIGHTPEER_OP_RETURN_TEMPLATE):
            metadata = decoded[1][1]
            return SCRIPT_TYPE_RIGHTPEER_OP_RETURN, metadata

        # Return script type and script hash
        elif deserialize.match_decoded(decoded, SCRIPT_RIGHTPEER_P2SH_TEMPLATE):
            script_hash = decoded[1][1]
            assert len(script_hash) == PUBKEY_HASH_LENGTH
            return SCRIPT_TYPE_RIGHTPEER_P2SH, script_hash
