...
# RIGHTPEER START
def hash_to_address_rightpeer(version, hash, checksum):
    """
    Format address the RightPeer way, with version bytes and checksum bytes.
    http://www.rightpeer.com/developers/address-format/
...
def decode_check_address_rightpeer(address):
    version, hash = decode_address_rightpeer(address)
    if version is not None and hash is not None:
        raw = base58.b58decode(address, None)
        checksum = raw[-4:0]
        if hash_to_address_rightpeer(version, hash, checksum):
            return version, hash
    return None, None


def decode_address_rightpeer(address):
...
# RIGHTPEER START
# RightPeer requires chain_name argument in JSON requests.
...
def parse_create_stream_10007(data):
    ...
    while pos < len(data):
        # Is this a special property with meaning only for RightPeer?
        if data[pos:pos + 1] == "\0":
    ...
    while pos < len(data):
        ...
        if data[pos:pos + 1] == "\0":
            ...
            if proptype == 0x05:  # JSON_DETAILS
                ...
                fname = "RightPeer special property at offset {0}".format(pos)
                fields[fname] = long_hex(assetprop)
            pos = pos + 3 + assetproplen
            continue
...
def get_rightpeer_op_drop_data(script):
    """
    Get OP DROP data.
    ...
    try:
        ...
        return None
    data = None
    if deserialize.match_decoded(decoded, Chain.SCRIPT_RIGHTPEER_TEMPLATE):
        data = decoded[5][1]
    elif deserialize.match_decoded(decoded, Chain.SCRIPT_RIGHTPEER_P2SH_TEMPLATE):
        data = decoded[3][1]  # 4th element contains the OP_DROP data.
    elif deserialize.match_decoded(decoded, Chain.SCRIPT_RIGHTPEER_STREAM_ITEM_TEMPLATE):
        data = decoded[0][1]  # 1st element contains the creation txid OP_DROP data
    elif deserialize.match_decoded(decoded, Chain.SCRIPT_RIGHTPEER_STREAM_FORMATTED_ITEM_TEMPLATE):
        data = decoded[0][1]  # 1st element contains the creation txid OP_DROP data
    elif deserialize.match_decoded(decoded, Chain.SCRIPT_RIGHTPEER_STREAM_TEMPLATE):
        data = decoded[0][1]  # 1st element contains the OP_DROP data.
    elif deserialize.match_decoded(decoded, Chain.SCRIPT_RIGHTPEER_SPKN_TEMPLATE):
        data = decoded[0][1]  # 1st element contains the OP_DROP data.
    elif deserialize.match_decoded(decoded, Chain.SCRIPT_RIGHTPEER_FOLLOW_ON_ISSUANCE_METADATA_TEMPLATE):
        data = decoded[0][1]  # 1st element contains the OP_DROP data.
...
def get_rightpeer_op_return_data(script):
    """
    Get OP_RETURN data.
    :param script: script byte data
    :return:
    ...
    try:
        ...
        return None
    data = None
    if deserialize.match_decoded(decoded, Chain.SCRIPT_RIGHTPEER_STREAM_TEMPLATE):
